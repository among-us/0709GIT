  $(function(){
   	$('#license_yes').click(function(){
        	...
        	var pl_license_cnt = $('#pl_cnt').text();	// tobe static
        	var tpl_license_cnt = $('#tpl_cnt').text();	// tobe dynamic
			...
			if(asispl > tobepl || asistpl > tobetpl ){
				Swal.fire('','등록할 개수가 허용 개수보다 적을 경우, 현재 허용된 Agent 개수를 등록할 개수에 맞게 조정해야 합니다','error');
			}else{
		...
		console.log(value);
	          ...
	          $.ajax({
			            ...
			            if(a == 1){
				            	...
				            	$('#asispl').text(result.data[8]);
				            	$('#tpl_cnt').text(result.data[4]);
				            	$('#asistpl').text(result.data[9]);
				            	$('#limited_url').text(result.data[6]);
				            	$('#integrity').text(result.data[7]);
				            	$('#maxST').text(result.data[3]);
				            	$('#maxDY').text(result.data[5]);
