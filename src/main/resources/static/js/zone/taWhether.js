            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, full, meta) { 
	                    if(full.allow_state == "1"){// 해당되는 버튼 하나만
		                       return  '<button id="rejectBtn" style="border : 1px solid #ccc" class="btn btn-nv mr-3" onclick= "reject( \'' + full.taa_ip+'\''+"\,'"+full.taa_hostname+'\' )">거부</button>';
		                       //'<button disabled id="allowBtn"  class="btn btn-dark"      onclick=  "allow( \'' + full.taa_ip +'\')">허용</button>'+
	                    }//if end test
	                    else{
	                    	 return '<button id="allowBtn" style="border : 1px solid #ccc" class="btn btn-nv mr-3" onclick=  "allow( \'' + full.taa_ip+'\''+"\,'"+full.taa_hostname+'\' )">허용</button>';
...
function tmLicense(zone_no){
	...
	function allow(taa_ip, taa_hostname) {
		var zone_no = $('#zone_no').val();
		var allow_state = 1;
		var taa_ip = taa_ip;
		...
		allowObj = new allowDTO(zone_no, allow_state, taa_ip,taa_hostname);
	...
	function reject(taa_ip,taa_hostname) {
		var taa_ip = taa_ip
		var zone_no = $('#zone_no').val();
		var reject_state = 2;
		rejectObj = new rejectDTO(zone_no, reject_state ,taa_ip, taa_hostname);
