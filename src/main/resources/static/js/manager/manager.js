...
$(function(){
	...
	allowTamName = '';
			...
			var tam_adm_port = $("input[name='tam_adm_port']").val();
			...
			*/
			...
			if(tam_local_ip == "" || tam_local_port == "" || tam_adm_port == ""){
			...
			}else if(tam_adm_port > 65535 || tam_adm_port<0){
				...
				if(tam_pub_adm_port != ""){
					var pub_admin_result = ValidatePort(tam_pub_adm_port);
					if(pub_admin_result == false){
						Swal.fire('입력오류','올바른 관리자 외부 포트를 입력바랍니다','error');
						return;						
					}
					
					if (tam_pub_adm_port > 65535 || tam_pub_adm_port < 0){
						Swal.fire('입력오류','올바른 범위의 관리자 외부 포트를 입력바랍니다','error');
						return;
					}
				}
				
					enrollObj = new EnrollDTO(tam_name,site_info,tam_local_ip,tam_local_port,tam_adm_port,tam_pub_ip,tam_pub_port,tam_pub_adm_port,shared_path,comm_type,watchdog);
		...
		$('#updateBtn').click(function() {
			...
			if(tam_local_ip == "" || tam_local_port == "" || tam_adm_port == ""){
				...
				updateObj = new UpdateDTO($("input[name='tam_no']").val(),
					...
					$("input[name='tam_pub_ip']").val(),
					$("input[name='tam_pub_port']").val(),
					$("input[name='tam_pub_adm_port']").val(),
