...
public class ZoneException {
	...
	CommonResponseDTO<Object> sQLIntegrityConstraintViolationException(
	...
	@ExceptionHandler(TNAuthException.class)
	CommonResponseDTO<Object> TNAuthException(TNAuthException e) {
		log.warn("{JNI ERROR}", e.getClass());
		ZoneResultEnum err = ZoneResultEnum.JNI_ERROR;
		ZoneInfoResponseDTO resDTO = new ZoneInfoResponseDTO(err);
		return CommonResponseDTO.builder().data(resDTO).build();
	}
