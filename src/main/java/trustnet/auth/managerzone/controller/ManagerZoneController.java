...
public class ManagerZoneController {
	...
	public ModelAndView managerZoneEnrollPage(
			@CookieValue(value = "UNETAUTHTOKEN", defaultValue = "") String token,
			ManagerZoneInfoDTO dto) {
		...
		ManagerZoneInfoVO managerZoneInfoVo = modelMapper.map(dto,
				ManagerZoneInfoVO.class);
		List<ManagerZoneMatchInfoVO> voList = managerZoneService
				.findAllHavingZone(managerZoneInfoVo);
		List<ManagerZoneMatchInfoDTO> afterZoneInfoDTOList = modelMapper.map(voList,
				new TypeToken<List<ManagerZoneMatchInfoDTO>>() {
				}.getType());

		ManagerInfoVO managerInfoVO = managerZoneService.findManager(managerZoneInfoVo);
		ManagerInfoDTO managerInfoDTO = modelMapper.map(managerInfoVO,
				ManagerInfoDTO.class);
		List<ZoneInfoVO> zoneVoList = zoneService.findAllZoneListOnlyExist();
		List<ZoneInfoDTO> beforeZoneInfoDTOList = modelMapper.map(zoneVoList,
