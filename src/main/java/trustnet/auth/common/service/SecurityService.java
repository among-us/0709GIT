...
public class SecurityService {
	 ...
	 private final long ttlMillis = 1800 * 1000;
	 
	 public SecurityAuthInfoVO createToken(SecurityAuthInfoVO authInfoVO) throws JsonProcessingException {
		 String tokenInputValue = this.objectMapper.writeValueAsString(authInfoVO);
		 log.info("tokenInputValue ==> {}",tokenInputValue);
		 String value = makeToken(tokenInputValue);
	     SecurityAuthInfoVO vo = new SecurityAuthInfoVO(true, value);
	     return vo;
	 }
	 
	 public SecurityAuthInfoVO checkTokenValidation(HttpServletRequest request) throws JsonParseException, JsonMappingException, IOException {
		 ...
		 SecurityAuthInfoVO retVO = tokenSub.length() > 0 ? 
				 objectMapper.readValue(tokenSub, SecurityAuthInfoVO.class) : new SecurityAuthInfoVO(false, null);
	 ...
	 public boolean checkAdminValidation(String tokenValue) throws JsonMappingException, JsonProcessingException {
		String tokenSub = tokenValue.length() > 0  ? getSubject(tokenValue) : "";
		SecurityAuthInfoVO retVO = null;
		retVO = tokenSub.length() > 0 ? 
				 objectMapper.readValue(tokenSub, SecurityAuthInfoVO.class) : new SecurityAuthInfoVO(false, null);
		 
		 boolean result = retVO.getPermissions().equalsIgnoreCase("admin") ? true : false;
		 return result;
	 }
	 ...
	 private String getTokenValue(HttpServletRequest request) {
		 ...
		 if(tokenVal.length() <= 0 || tokenVal == null)  return "";
		 return tokenVal;
	 }
